...
Every graphic here is generated inside this repository rather than loaded from a third-party card service. `ascii.svg` is built from the profile avatar by [`scripts/make_avatar_ascii.py`](scripts/make_avatar_ascii.py). The contribution graphics and section headings are generated by [`scripts/generate_stats.py`](scripts/generate_stats.py) from GitHub’s GraphQL API, then refreshed by [a scheduled GitHub Action](.github/workflows/stats.yml).
